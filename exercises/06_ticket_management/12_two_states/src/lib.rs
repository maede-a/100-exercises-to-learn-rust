...
impl TicketStore {
    ...
    pub fn add_ticket(&mut self, ticket_draft: TicketDraft) -> TicketId{
        ...
        let ticket = Ticket { id: id, title: ticket_draft.title, description: ticket_draft.description, status: Status::ToDo };
        self.tickets.push(ticket);
        id
    }

    pub fn get(&self, id: TicketId) -> Option<&Ticket> {
        if self.tickets.len() as u64 > id.0 {
            return Some(&self.tickets[id.0 as usize])
        } else {
            return None
        }
    }
